class ReelView {
  constructor(mapTextures, symbolTextures, spineData, symbolConfig, mapDimensions, mapNames, symbolNames, reelset) {
    ...
    // Add background sprite after map sprites to render on top
  ...
  startScrollToMap(targetIndex, duration = 2000) {
    ...
    // Show background when spinning starts
    if (this.backgroundSprite) {
      this.backgroundSprite.visible = true;
      console.log('Background shown during spin');
    }
    ...
    // Background animation: always scroll top-to-bottom
    const bgStartY = this.backgroundSprite.y;
    const bgEndY = bgStartY + this.canvasHeight; // Scroll down
    ...
    // 6. Zoom Out and settle camera after scroll
    gsap.to(this.mapContainer.scale, {
      x: 1,
      y: 1,
      duration: 0.5,
  ...
  completeScroll(targetIndex) {
    // Hide background when spin stops
    if (this.backgroundSprite) {
      this.backgroundSprite.visible = false;
      this.backgroundSprite.y = 0; // Reset background position
      console.log('Background hidden and position reset after spin');
    }
